    ...
    def updateVisuals(self):
        ...
        for i, c in enumerate(self._grid):
            ...
            for j, o in enumerate(c):
                ...
                self.helpergrid.append(val)
                self.itemconfigure(o, fill=c.toHex())

        print(self.helpergrid)

        minIndex= self.helpergrid.index(min(self.helpergrid))

        f=open("index.txt", "w")
        f.write(str(minIndex)+"\n")
        f.close()

        print("Received minIndex is: ", minIndex)
        minVal=str(minIndex)
        bminVal = bytes(minVal, encoding='utf-8')
        ...
        #for p in ports:
         ...
         #   print('Checking port %s / %s' % (p[0], p[1]))
          #  if "ttyACM0" in p[1]:  # Find "ardUINO" and "genUINO" boards
           #     port = p
            #    break
